...
class Grid
{
  ...
  static typename std::map<int,T*>::size_type cache_size;

public:
  /** The cache_limit defaults to 360. Adjust it to set the number of 
   *  grid squares to store in memory. */
  static typename std::map<int,T*>::size_type cache_limit;

  Grid(GridSquare::height_t sea_level_rise_ =0)
  : sea_level_rise(sea_level_rise_)
  {
  ...
  GridSquare::height_t height(float longitude, float latitude) const
  {
    int lng,lat;
    ...
    int k =T::key(lng,lat);
    typename std::map<int,T*>::iterator pos =cache.find(k);
    if(pos==cache.end())
    {
  ...
  {
    GridSquare::height_t h =height(longitude,latitude);
    return( h!=GridSquare::NO_DATA && h<=sea_level_rise );
  }
...
private:
  ...
  {
    std::cerr<<"clear_cache"<<std::endl;
    typename std::map<int,T*>::iterator curr,next=cache.begin();
    while(next!=cache.end())
    {
      curr=next++;
      if( curr->second->has_data() )
      {
        delete curr->second;
        cache.erase(curr);
        --cache_size;
  ...
  GridSquare::height_t sea_level_rise;
};

template<class T> std::map<int,T*>                     Grid<T>::cache;
template<class T> typename std::map<int,T*>::size_type Grid<T>::cache_size =0;
template<class T> typename std::map<int,T*>::size_type Grid<T>::cache_limit=360;
